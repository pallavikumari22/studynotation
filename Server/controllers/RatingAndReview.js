...
exports.getAllRating=async(req,res)=>{
  try{
    ...
    const allReviews=await RatingAndReview.find({ status: { $ne: "Hidden" } })
                    .sort({
                      rating:"desc",
                    }).populate({
                      path:"user",
...
exports.getModerationReviews=async(req,res)=>{
  try{
    const reviews=await RatingAndReview.find({})
      .sort({ reportCount: "desc", createdAt: "desc" })
...
exports.reportReview=async(req,res)=>{
  try{
    const { reviewId, reason = "Reported by learner" }=req.body;
